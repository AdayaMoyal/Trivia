...
public:
	...
	static int convert4BytesToInt(char*);
	static int getMessageCode(char*);
	static std::string getDataFromMessage(char*);
	static char* vectorToCharArray(std::vector<unsigned char>);
	static std::vector<unsigned char> stringToVector(std::string);
	static std::string CharArrayToString(char*);
	static int lengthOfArray(char*);
